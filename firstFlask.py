...
@app.route('/v1/form', methods=['POST'])
def form_action():
    client = ndb.Client()
    with client.context():
        ...
        print(book.key.id())
        book.put()
    return render_template('home.html', id=book.key.id())
...
def delete(book_id):
    ...
    with client.context():
        # query1 = Book.query()
        # query2 = query1.filter(Book.key.id() == book_id)
        # print(type(query2))
        # for book in query2:
        #     print(book)
        #     print(type(book))
    ...
    with client.context():
        ...
        for book in results:
            books_db.append({'book_id': book.key.id(),
                             ...
                             'isbn': book.isbn})
    return jsonify(success=True, books=books_db, more=more, next_cursor=cursor.urlsafe().decode(), curr_cursor=prev_cursor)
